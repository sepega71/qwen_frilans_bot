...
# -*- coding: utf-8 -*-
"""
Модуль сбора данных с freemarket.ru
Реализует функциональность для получения проектов с freemarket.ru через веб-скрапинг.
"""
...
class FreemarketCollector:
    """
    Класс для сбора данных с freemarket.ru
    Поддерживает получение данных через веб-скрапинг
    """
    
    def __init__(self):
        """Инициализация сборщика данных с freemarket.ru"""
        self.base_url = "https://freemarket.ru"
    ...
    async def fetch_projects_web(self, max_pages: int = 3) -> List[Dict[str, Any]]:
        """
        Сбор проектов с freemarket.ru через веб-скрапинг
        ...
        try:
            ...
            self.logger.error(f"Ошибка при сборе данных с freemarket.ru через веб-скрапинг: {e}")
    ...
    def _parse_project_element(self, element) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Извлекаем регион
            region_elem = element.find('span', class_='project-region')
            if region_elem:
                project['region'] = region_elem.get_text(strip=True)
            
            # Извлекаем дату
            date_elem = element.find('time', class_='project-date')
            if date_elem:
            ...
            if project:
                project['source'] = 'freemarket.ru'
                project['type'] = 'order'
                return project
        except Exception as e:
            self.logger.error(f"Ошибка при парсинге элемента проекта freemarket.ru: {e}")
    ...
    def normalize_project_data(self, project: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            project: Проект в формате freemarket.ru
        ...
        normalized = {
            ...
            'source': project.get('source', 'freemarket.ru'),
